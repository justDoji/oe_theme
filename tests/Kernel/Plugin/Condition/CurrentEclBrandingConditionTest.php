...
class CurrentEclBrandingConditionTest extends AbstractKernelTestBase {
  ...
  public function testCondition(): void {
    ...
    $condition = $manager->createInstance('oe_theme_helper_current_ecl_branding');
    $condition->setConfiguration(['ecl_branding' => 'standardised']);

    $condition_empty = $manager->createInstance('oe_theme_helper_current_ecl_branding');

    $condition_negated = $manager->createInstance('oe_theme_helper_current_ecl_branding');
    $condition_negated->setConfiguration(['ecl_branding' => 'standardised', 'negate' => TRUE]);

    $this->assertEqual($condition->summary(), new FormattableMarkup('The current ECL branding is @ecl_branding', ['@ecl_branding' => 'standardised']));
    $this->assertEqual($condition_empty->summary(), new FormattableMarkup('The current ECL branding can be set to anything', []));
    $this->assertEqual($condition_negated->summary(), new FormattableMarkup('The current ECL branding is not @ecl_branding', ['@ecl_branding' => 'standardised']));
